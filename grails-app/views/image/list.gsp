...
<div id="list-image" class="content scaffold-list" role="main">
    ...
    <table>
        ...
        <tr>
            ...
            <g:sortableColumn property="album" title="Album"/>
        </tr>
        </thead>
        <tbody>
        <g:each in="${imageList}" status="i" var="imageInstance">
            <tr class="${i % 2 == 0 ? "even" : "odd"}" style="height: ${Constants.thumbnailSize}px;">
                <td class="image-column"><img
                        src="${imageInstance?.getShowImageUrl(Constants.thumbnailSize, Constants.thumbnailSize).url}">
                </td>
                ...
                <td class="album-column"><g:link controller="album" action="show"
                                                 id="${imageInstance.album.id}">${imageInstance.album.name}</g:link></td>
            </tr>
        </g:each>
        </tbody>
    </table>
